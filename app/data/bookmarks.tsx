// https://opensheet.elk.sh/1jonPSUsmPe5NZ9odeGyrgt8I32oViHkQ79XFVYyv2ZU/Bookmarks

const bookmarks = [
  {
    Title: 'Extract Pics',
    Link: 'https://extract.pics',
    Description: 'Extract all images from a web page for free.',
    Tags: 'bulk downloader, images from url',
    Thumbnail: '/assets/thumbnails/extract.pics.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/extract.pics.png',
  },
  {
    Title: 'Photopea',
    Link: 'https://www.photopea.com/',
    Description: 'Run photoshop for free online without installing it.',
    Tags: 'photoshop, image editor, psd editor',
    Thumbnail: '/assets/thumbnails/photopea.com.webp',
    Category: 'Tool',
    Favicon: '/assets/favicons/photopea.com.png',
  },
  {
    Title: 'Vectorpea',
    Link: 'https://www.vectorpea.com/',
    Description: 'Run illustrator for free online without installing it.',
    Tags: 'illustrator, vector editor, eps editor',
    Thumbnail: '/assets/thumbnails/vectorpea.com.webp',
    Category: 'Tool',
    Favicon: '/assets/favicons/vectorpea.com.png',
  },
  {
    Title: 'Monkey Type',
    Link: 'https://monkeytype.com/',
    Description: 'Typing test: adjust modes, monitor growth, boost speed.',
    Tags: 'typing speed, typing practice',
    Thumbnail: '/assets/thumbnails/monkeytype.com.webp',
    Category: 'Productivity',
    Favicon: '/assets/favicons/monkeytype.com.png',
  },
  {
    Title: 'Hemingway App',
    Link: 'https://hemingwayapp.com/',
    Description: 'Clarifies writing, marking long sentences & errors.',
    Tags: 'writing editor, grammer checker, proof reader',
    Thumbnail: '/assets/thumbnails/hemingwayapp.com.webp',
    Category: 'Productivity',
    Favicon: '/assets/favicons/hemingwayapp.com.png',
  },
  {
    Title: 'Witeboard',
    Link: 'https://witeboard.com/',
    Description: 'Witeboard is the fastest real-time online whiteboard.',
    Tags: 'whiteboard, brainstorming',
    Thumbnail: '/assets/thumbnails/witeboard.com.webp',
    Category: 'Tool',
    Favicon: '/assets/favicons/witeboard.com.png',
  },
  {
    Title: 'Excalidraw',
    Link: 'https://excalidraw.com/',
    Description: 'Whiteboard tool that lets you easily sketch diagrams.',
    Tags: 'whiteboard, brainstorming',
    Thumbnail: '/assets/thumbnails/excalidraw.com.webp',
    Category: 'Tool',
    Favicon: '/assets/favicons/excalidraw.com.png',
  },
  {
    Title: 'Littler Books',
    Link: 'https://littlerbooks.com/',
    Description: 'Nonfiction books summarized into their essential ideas.',
    Tags: 'books to read, book suggestions, book summaries',
    Thumbnail: '/assets/thumbnails/littlerbooks.com.webp',
    Category: 'Learning',
    Favicon: '/assets/favicons/littlerbooks.com.png',
  },
  {
    Title: 'Quran',
    Link: 'https://quran.com/',
    Description:
      'Simple Quran with translation available in multiple languages.',
    Tags: 'quran recitation',
    Thumbnail: '/assets/thumbnails/quran.com.webp',
    Category: 'Reading',
    Favicon: '/assets/favicons/quran.com.png',
  },
  {
    Title: 'WordStream',
    Link: 'https://www.wordstream.com/keywords',
    Description: 'Fast and easy way to find keywords.',
    Tags: 'seo, search engine optimization',
    Thumbnail: '/assets/thumbnails/wordstream.com.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/wordstream.com.png',
  },
  {
    Title: 'Fast',
    Link: 'https://fast.com/',
    Description: 'Test internet speed without hustle.',
    Tags: 'wifi, router, network',
    Thumbnail: '/assets/thumbnails/fast.com.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/fast.com.png',
  },
  {
    Title: 'Growth Design',
    Link: 'https://growth.design/',
    Description: 'Level up your product skills with bite-size tips.',
    Tags: 'pyschology, uiux',
    Thumbnail: '/assets/thumbnails/design.growth.webp',
    Category: 'Learning',
    Favicon: '/assets/favicons/design.growth.png',
  },
  {
    Title: 'HTTPIE',
    Link: 'https://httpie.io/app',
    Description: 'REST API testing tool. Postman alternative.',
    Tags: 'coding, javascript, react',
    Thumbnail: '/assets/thumbnails/httpie.io.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/httpie.io.png',
  },
  {
    Title: 'mymind',
    Link: 'https://access.mymind.com/everything',
    Description: 'Save images, videos & links with AI organize.',
    Tags: 'bookmarks, collections, saved',
    Thumbnail: '/assets/thumbnails/mymind.com.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/mymind.com.png',
  },
  {
    Title: 'Curation of Curations',
    Link: 'https://www.curationofcurations.com/',
    Description: 'The ultimate design curations to get inspired.',
    Tags: 'user experience, design',
    Thumbnail: '/assets/thumbnails/curationofcurations.com.webp',
    Category: 'Collection',
    Favicon: '/assets/favicons/curationofcurations.com.png',
  },
  {
    Title: 'Eleven Labs',
    Link: 'https://elevenlabs.io/',
    Description: 'Create the most realistic speech with our AI audio.',
    Tags: 'text to speech, ai',
    Thumbnail: '/assets/thumbnails/elevanlabs.io.webp',
    Category: 'Tool',
    Favicon: '/assets/favicons/elevanlabs.io.png',
  },
  {
    Title: 'Uppbeat',
    Link: 'https://uppbeat.io/',
    Description: 'Free music and sound effect for video editors.',
    Tags: 'audio, sounds, music',
    Thumbnail: '/assets/thumbnails/uppbeat.io.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/uppbeat.io.png',
  },
  {
    Title: 'Navbar Gallery',
    Link: 'https://www.navbar.gallery/',
    Description: 'Collection of best Navbars.',
    Tags: 'website design, components',
    Thumbnail: '/assets/thumbnails/navbar.gallery.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/navbar.gallery.png',
  },
  {
    Title: 'Footer Design',
    Link: 'https://footer.design/',
    Description: 'Curated gallery of top website footer inspiration.',
    Tags: 'website design, components',
    Thumbnail: '/assets/thumbnails/footer.design.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/footer.design.png',
  },
  {
    Title: 'App Motion Design',
    Link: 'https://appmotion.design/',
    Description: 'Explore the best, hand-picked app motion design.',
    Tags: 'micro-interactions, animations, app design',
    Thumbnail: '/assets/thumbnails/appmotion.design.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/appmotion.design.png',
  },
  {
    Title: '60fps',
    Link: 'https://www.60fps.design/',
    Description: 'Collection of delightful interaction in apps.',
    Tags: 'micro-interactions, animations, app design',
    Thumbnail: '/assets/thumbnails/60fps.design.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/60fps.design.png',
  },
  {
    Title: 'Shots',
    Link: 'https://shots.so/',
    Description: 'Create amazing mockups for screenshots and more online.',
    Tags: 'screenshot, mockup',
    Thumbnail: '/assets/thumbnails/shots.so.webp',
    Category: 'Tool',
    Favicon: '/assets/favicons/shots.so.png',
  },
  {
    Title: 'Tweek',
    Link: 'https://tweek.so/',
    Description: 'User-friendly weekly to-do list in a calendar format.',
    Tags: 'calendar, todolist',
    Thumbnail: '/assets/thumbnails/tweek.so.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/tweek.so.png',
  },
  {
    Title: 'UI Colors',
    Link: 'https://uicolors.app/create',
    Description: 'Get hue values of any color. From one shade to another.',
    Tags: 'palettes, colors',
    Thumbnail: '/assets/thumbnails/uicolors.app.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/uicolors.app.png',
  },
  {
    Title: 'Octopus',
    Link: 'https://octopus.do/',
    Description: 'Visual sitemap builder with lo-fi wireframes.',
    Tags: 'seo, search engine optimization, web design',
    Thumbnail: '/assets/thumbnails/octopus.do.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/octopus.do.png',
  },
  {
    Title: 'Teamcamp',
    Link: 'https://www.teamcamp.app/',
    Description: 'Manage projects, collaborate with your team.',
    Tags: 'project management',
    Thumbnail: '/assets/thumbnails/teamcamp.app.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/teamcamp.app.png',
  },
  {
    Title: 'RaySo',
    Link: 'https://ray.so/',
    Description: 'Turn code to pics. Pic modes, change colors and sizes.',
    Tags: 'screenshot, social media post',
    Thumbnail: '/assets/thumbnails/ray.so.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/ray.so.png',
  },
  {
    Title: 'MuscleWiki',
    Link: 'https://musclewiki.com/',
    Description:
      'Interactive body map with 2000+ guided exercises for all fitness levels.',
    Tags: 'fitness, exercises',
    Thumbnail: '/assets/thumbnails/musclewiki.com.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/musclewiki.com.png',
  },
  {
    Title: 'MergeMyPDFs',
    Link: 'https://mergemypdfs.com/',
    Description: "Combine PDFs in a snap. It's quick and easy to use.",
    Tags: 'pdf',
    Thumbnail: '/assets/thumbnails/mergemypdfs.com.webp',
    Category: 'Tool',
    Favicon: '/assets/favicons/mergemypdfs.com.png',
  },
  {
    Title: 'Index Guru',
    Link: 'https://www.indexguru.io/',
    Description: '#1 Website indexing tool. Index unindexed pages.',
    Tags: 'seo, indexing',
    Thumbnail: '/assets/thumbnails/indexguru.com.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/indexguru.com.png',
  },
  {
    Title: 'Logo System',
    Link: 'https://logosystem.co/',
    Description:
      'Find inspiration by exploring this library of real and fictional logos.',
    Tags: 'logo, graphic design',
    Thumbnail: '/assets/thumbnails/logosystem.co.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/logosystem.co.png',
  },
  {
    Title: 'Coolors',
    Link: 'https://coolors.co/palettes/trending',
    Description: 'Thousands of beautiful color palettes inspiration.',
    Tags: 'colors, colours',
    Thumbnail: '/assets/thumbnails/coolors.co.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/coolors.co.png',
  },
  {
    Title: 'Shortcuts Design',
    Link: 'https://shortcuts.design/',
    Description: 'Every shortcut for designers, centralized & searchable.',
    Tags: 'shortcuts, softwares',
    Thumbnail: '/assets/thumbnails/shortcuts.design.webp',
    Category: 'Productivity',
    Favicon: '/assets/favicons/shortcuts.design.png',
  },
  {
    Title: 'Daily Dev',
    Link: 'https://app.daily.dev/',
    Description: 'Quality knowledge about tech, design & development.',
    Tags: 'learning, programming, coding',
    Thumbnail: '/assets/thumbnails/app.daily.dev.webp',
    Category: 'Learning',
    Favicon: '/assets/favicons/app.daily.dev.png',
  },
  {
    Title: 'Uiverse',
    Link: 'https://uiverse.io/',
    Description: 'Free UI elements made with CSS.',
    Tags: 'components, elements',
    Thumbnail: '/assets/thumbnails/uiverse.io.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/uiverse.io.png',
  },
  {
    Title: 'Design Checklist',
    Link: 'https://www.checklist.design/',
    Description: 'List of checklists for user interface design.',
    Tags: 'checklists, ui design',
    Thumbnail: '/assets/thumbnails/checklist.design.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/checklist.design.png',
  },
  {
    Title: 'Gridzzly',
    Link: 'https://gridzzly.com/',
    Description: 'Make printable grid paper easily.',
    Tags: 'printable paper, grids, layout',
    Thumbnail: '/assets/thumbnails/gridzzly.com.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/gridzzly.com.png',
  },
  {
    Title: 'StoryMap',
    Link: 'https://storymap.site/',
    Description: 'Effortless user story mapping.',
    Tags: 'brainstorming, mapping, planning',
    Thumbnail: '/assets/thumbnails/storymap.site.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/storymap.site.png',
  },
  {
    Title: 'Design Spells',
    Link: 'https://www.designspells.com/',
    Description: 'Design details that feel like magic.',
    Tags: 'micro-interactions, design details',
    Thumbnail: '/assets/thumbnails/designspells.com.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/designspells.com.png',
  },
  {
    Title: 'Thiings',
    Link: 'https://www.thiings.co/things',
    Description: 'Collection of AI-generated, skeuomorphic icons.',
    Tags: 'icons',
    Thumbnail: '/assets/thumbnails/thiings.co.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/thiings.co.png',
  },
  {
    Title: 'Colorway',
    Link: 'https://colorway.app/',
    Description: 'Eyedropper, color picker, and palette generator.',
    Tags: 'colors, colours, tints, shades, hues',
    Thumbnail: '/assets/thumbnails/colorway.app.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/colorway.app.png',
  },
  {
    Title: 'Picular',
    Link: 'https://picular.co/',
    Description: 'Generate colors based on anything like color of tiger.',
    Tags: 'colors, colours, color generator, colors by search',
    Thumbnail: '/assets/thumbnails/picular.co.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/picular.co.png',
  },
  {
    Title: 'SVGL',
    Link: 'https://svgl.app/',
    Description: 'SVG logos of top tech brands and companies.',
    Tags: 'logos, companies, brand',
    Thumbnail: '/assets/thumbnails/svgl.app.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/svgl.app.png',
  },
  {
    Title: 'Box Shadows',
    Link: 'https://getcssscan.com/css-box-shadow-examples',
    Description: '95 Beautiful CSS box-shadow examples.',
    Tags: 'box shadow, css, styles',
    Thumbnail: '/assets/thumbnails/box-shadows.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/box-shadows.png',
  },
  {
    Title: 'Pomofocus',
    Link: 'https://pomofocus.io/',
    Description: 'Online pomodoro app to help focus on tasks.',
    Tags: 'pomodoro, timer, clock, task management, productivity',
    Thumbnail: '/assets/thumbnails/pomofocus.io.webp',
    Category: 'Productivity',
    Favicon: '/assets/favicons/pomofocus.io.png',
  },
  {
    Title: 'Designer Daily Report',
    Link: 'https://designerdailyreport.com/',
    Description: 'Curated newspaper for the designers.',
    Tags: 'design, uiux, colors, inspiration, images, icons',
    Thumbnail: '/assets/thumbnails/designerdailyreport.com.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/designerdailyreport.com.png',
  },
  {
    Title: 'Prototypr',
    Link: 'https://prototypr.io/',
    Description:
      'Share your work, join the conversation, connect with creators.',
    Tags: 'design, resources, community, uiux, prototyping',
    Thumbnail: '/assets/thumbnails/prototypr.io.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/prototypr.io.png',
  },
  {
    Title: 'Best Designs on X',
    Link: 'https://bestdesignsonx.com/',
    Description: 'Inspire by top designs posted on X.',
    Tags: 'resources, design, uiux, prototyping, inspiration',
    Thumbnail: '/assets/thumbnails/bestdesignsonx.com.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/bestdesignsonx.com.png',
  },
  {
    Title: '10015',
    Link: 'https://10015.io/',
    Description: 'All the tools you needed in one place.',
    Tags: 'tools, utlity, typography, colors, images, editting, coding, programming, development',
    Thumbnail: '/assets/thumbnails/10015.io.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/10015.io.png',
  },
  {
    Title: 'Easing',
    Link: 'https://www.easing.dev/',
    Description: 'A curated collection of easing graphs.',
    Tags: 'micro-interactions, animations, css',
    Thumbnail: '/assets/thumbnails/easing.dev.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/easing.dev.png',
  },
  {
    Title: 'Magic UI',
    Link: 'https://magicui.design/',
    Description: 'Collection of beautiful uiux components.',
    Tags: 'uiux, components, ui elements, design',
    Thumbnail: '/assets/thumbnails/magicui.design.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/magicui.design.png',
  },
  {
    Title: 'Microcopy',
    Link: 'https://www.microcopy.me/',
    Description: '500+ carefully curated microcopies for your website.',
    Tags: 'copywriting, marketing, design pyschology',
    Thumbnail: '/assets/thumbnails/microcopy.me.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/microcopy.me.png',
  },
  {
    Title: 'Landingfolio',
    Link: 'https://www.landingfolio.com/',
    Description:
      'Landing page inspiration, figma, tailwind & webflow components.',
    Tags: 'figma, components, tailwind, landing pages, webflow, ai, artificial intelligence',
    Thumbnail: '/assets/thumbnails/landingfolio.com.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/landingfolio.com.png',
  },
  {
    Title: 'Serpdrill',
    Link: 'https://www.serpdrill.com/',
    Description: 'AI-powered keyword research and SERP analysis tool.',
    Tags: 'seo tool, keyword research, serp analysis, ai content, niche targeting',
    Thumbnail: '/assets/thumbnails/serpdrill.com.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/serpdrill.com.png',
  },
  {
    Title: 'The Stocks',
    Link: 'https://thestocks.im/',
    Description: 'Royalty-free stock photos, icons, videos, mockups.',
    Tags: 'stock photos, mockups, icons, design assets',
    Thumbnail: '/assets/thumbnails/thestocks.im.webp',
    Category: 'Collection',
    Favicon: '/assets/favicons/thestocks.im.png',
  },
  {
    Title: 'Emoji Kitchen',
    Link: 'https://emoji.supply/kitchen/',
    Description: 'Mix and match emojis for fun combos.',
    Tags: 'emojis',
    Thumbnail: '/assets/thumbnails/emoji.supply_kitchen.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/emoji.supply_kitchen.png',
  },
  {
    Title: 'Logggos',
    Link: 'https://www.logggos.club/',
    Description: 'Catalog of well-design logos.',
    Tags: 'logo inspiration, branding, typography',
    Thumbnail: '/assets/thumbnails/logggos.club.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/logggos.club.png',
  },
  {
    Title: 'World Vector Logo',
    Link: 'https://worldvectorlogo.com/',
    Description: 'Download vector logos of brands you love.',
    Tags: 'vector logos, SVG downloads, brand logos',
    Thumbnail: '/assets/thumbnails/worldvectorlogo.com.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/worldvectorlogo.com.png',
  },
  {
    Title: 'Logo Book',
    Link: 'http://logobook.com/',
    Description: 'Discover the worlds finest logos, symbols & trademarks.',
    Tags: 'logos, inspiration',
    Thumbnail: '/assets/thumbnails/logobook.com.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/logobook.com.png',
  },
  {
    Title: 'Same Energy',
    Link: 'https://same.energy/',
    Description: 'Find similar images. Search based on visual instead of text.',
    Tags: 'search engine',
    Thumbnail: '/assets/thumbnails/same.energy.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/same.energy.png',
  },
  {
    Title: 'Killer Portfolio',
    Link: 'https://www.killerportfolio.com/',
    Description: 'Curated portfolio inspiration from top designers.',
    Tags: 'inspiration, portfolio, web design',
    Thumbnail: '/assets/thumbnails/killerportfolio.com.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/killerportfolio.com.png',
  },
  {
    Title: 'Brand Archive',
    Link: 'https://brandarchive.xyz/',
    Description: 'A searchable archive of real-world brand designs.',
    Tags: 'graphic design, branding, logo, inspiration',
    Thumbnail: '/assets/thumbnails/brandarchive.xyz.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/brandarchive.xyz.png',
  },
  {
    Title: 'Wireframe Examples',
    Link: 'https://www.wireframe-examples.com/',
    Description: 'Wireframe layouts and UI patterns for quick reference.',
    Tags: 'resources, design, uiux, prototyping, inspiration',
    Thumbnail: '/assets/thumbnails/wireframe-examples.com.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/wireframe-examples.com.png',
  },
  {
    Title: 'Fontshare',
    Link: 'https://www.fontshare.com/',
    Description: 'Free, high-quality fonts by Indian Type Foundry.',
    Tags: 'fonts, typography, free fonts, typeface library, design assets',
    Thumbnail: '/assets/thumbnails/fontshare.com.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/fontshare.com.png',
  },
  {
    Title: 'Uncut',
    Link: 'https://uncut.wtf/',
    Description: 'Curated free fonts from independent type designers.',
    Tags: 'indie fonts, type design, free typography, modern fonts, font collection',
    Thumbnail: '/assets/thumbnails/uncut.wtf.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/uncut.wtf.png',
  },
  {
    Title: 'Phosphor Icons',
    Link: 'https://phosphoricons.com/',
    Description:
      'Flexible, customizable icon set for designers and developers.',
    Tags: 'icons, design assets, svg, ui elements, vector',
    Thumbnail: '/assets/thumbnails/phosphoricons.com.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/phosphoricons.com.png',
  },
  {
    Title: 'Flagpack',
    Link: 'https://flagpack.xyz/',
    Description: 'Ready-to-use, customizable flag icons for digital projects.',
    Tags: 'flags, icons, svg, design assets, branding',
    Thumbnail: '/assets/thumbnails/flagpack.xyz.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/flagpack.xyz.png',
  },
  {
    Title: 'How much I charge?',
    Link: 'https://thenuschool.com/how-much/',
    Description: 'Simple calculator to set your freelance rates.',
    Tags: 'freelance, pricing, calculator, business, design rates',
    Thumbnail: '/assets/thumbnails/thenuschool.com.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/thenuschool.com.png',
  },
  {
    Title: 'Producthunt',
    Link: 'https://www.producthunt.com/',
    Description: 'Daily hub to discover new apps, tools, and startups.',
    Tags: 'startups, tools, inspiration, resources, tech',
    Thumbnail: '/assets/thumbnails/producthunt.com.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/producthunt.com.png',
  },
  {
    Title: 'tldraw',
    Link: 'https://www.tldraw.com/',
    Description:
      'Collaborative whiteboard for drawing, planning, and brainstorming.',
    Tags: 'whiteboard, brainstorming, collaboration, drawing, design tool',
    Thumbnail: '/assets/thumbnails/tldraw.com.webp',
    Category: 'Tool',
    Favicon: '/assets/favicons/tldraw.com.png',
  },
  {
    Title: 'Maker Stations',
    Link: 'https://www.makerstations.io/',
    Description: 'Inspiring workspaces and setups from creative professionals.',
    Tags: 'workspace, inspiration, setup, productivity, creative spaces',
    Thumbnail: '/assets/thumbnails/makerstations.io.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/makerstations.io.png',
  },
  {
    Title: 'Cofolios',
    Link: 'https://www.cofolios.com/',
    Description:
      'Curated collection of design intern portfolios from top tech companies.',
    Tags: 'design portfolios, inspiration, students, internships, uiux',
    Thumbnail: '/assets/thumbnails/cofolios.com.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/cofolios.com.png',
  },
  {
    Title: 'Country Flags',
    Link: 'https://www.linguana.io/resources/country-flags',
    Description: 'Free SVG country-flag icons for your projects.',
    Tags: 'country flags, icons, svg, assets, resources',
    Thumbnail: '/assets/thumbnails/linguana.io.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/linguana.io.png',
  },
  {
    Title: 'Patttterns',
    Link: 'https://patttterns.net/',
    Description: 'Open catalog of real-world UX/UI design patterns.',
    Tags: 'design patterns, UX patterns, UI components, interaction design, inspiration',
    Thumbnail: '/assets/thumbnails/patttterns.net.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/patttterns.net.png',
  },
  {
    Title: 'Postable',
    Link: 'https://www.postable.app/',
    Description: 'Convert tweets into engaging social media posts.',
    Tags: 'social media, content repurposing, copywriting, marketing',
    Thumbnail: '/assets/thumbnails/postable.app.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/postable.app.png',
  },
  {
    Title: 'Jitter',
    Link: 'https://jitter.video/',
    Description:
      'Fast, collaborative web tool for motion design and animation.',
    Tags: 'motion design, animation tool, UIUX, collaborative, Figma integration',
    Thumbnail: '/assets/thumbnails/jitter.video.webp',
    Category: 'Tool',
    Favicon: '/assets/favicons/jitter.video.png',
  },
  {
    Title: 'Roadmaps',
    Link: 'https://roadmap.sh/',
    Description:
      'Community-driven tech career and skill roadmaps for developers.',
    Tags: 'tech career, learning paths, developer roadmaps, guides, community-powered',
    Thumbnail: '/assets/thumbnails/roadmap.sh.webp',
    Category: 'Learning',
    Favicon: '/assets/favicons/roadmap.sh.png',
  },
  {
    Title: 'Learn Anything',
    Link: 'https://learn-anything.xyz/',
    Description:
      'Interactive learning resource maps—discover and explore any topic.',
    Tags: 'learning paths, knowledge maps, resource discovery, community-curated, topic exploration',
    Thumbnail: '/assets/thumbnails/learn-anything.xyz.webp',
    Category: 'Learning',
    Favicon: '/assets/favicons/learn-anything.xyz.png',
  },
  {
    Title: 'Storytale',
    Link: 'https://storytale.io/browse/?isFree=true',
    Description:
      'Free, high-quality illustrations & mockups for designers and creators.',
    Tags: 'free illustrations, design assets, mockups, vector graphics, creative resources',
    Thumbnail: '/assets/thumbnails/storytale.io.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/storytale.io.png',
  },
  {
    Title: 'Rive',
    Link: 'https://rive.app/',
    Description:
      'Design and ship interactive, real-time animations for apps and websites.',
    Tags: 'motion design, interactive animation, vector graphics, UIUX tool, export runtime',
    Thumbnail: '/assets/thumbnails/rive.app.webp',
    Category: 'Tool',
    Favicon: '/assets/favicons/rive.app.png',
  },
  {
    Title: 'Design Resources',
    Link: 'https://designresourc.es/',
    Description:
      'Curated directory of 300+ design tools, assets, and inspiration.',
    Tags: 'design resources, tools & assets, inspiration, UI/UX, free design directory',
    Thumbnail: '/assets/thumbnails/designresourc.es.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/designresourc.es.png',
  },
  {
    Title: 'Humane by Design',
    Link: 'https://humanebydesign.com/',
    Description:
      'Guidance for building humane, ethical digital products prioritizing user well-being.',
    Tags: 'ethical design, digital wellbeing, humane UX, design principles, inclusive design',
    Thumbnail: '/assets/thumbnails/humanebydesign.com.webp',
    Category: 'Learning',
    Favicon: '/assets/favicons/humanebydesign.com.png',
  },
  {
    Title: '30 Seconds of Code',
    Link: 'https://www.30secondsofcode.org/',
    Description: 'Quick code snippets and articles to sharpen dev skills fast.',
    Tags: 'programming, snippets, js tutorials, learning resources, web development',
    Thumbnail: '/assets/thumbnails/30secondsofcode.org.webp',
    Category: 'Learning',
    Favicon: '/assets/favicons/30secondsofcode.org.png',
  },
  {
    Title: 'Instant Domain Search',
    Link: 'https://instantdomainsearch.com/',
    Description:
      'Real-time domain name search with instant availability and suggestions.',
    Tags: 'domain search, domain generator, TLDs, premium domains, domain tools',
    Thumbnail: '/assets/thumbnails/instantdomainsearch.com.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/instantdomainsearch.com.png',
  },
  {
    Title: 'Marketer Gems',
    Link: 'https://www.marketergems.com/',
    Description:
      'Curated newsletter of top marketing insights, tools, and campaigns.',
    Tags: 'marketing newsletter, growth tips, tools & apps, campaigns breakdown, content curator',
    Thumbnail: '/assets/thumbnails/marketergems.com.webp',
    Category: 'Learning',
    Favicon: '/assets/favicons/marketergems.com.png',
  },
  {
    Title: 'Flowmapp',
    Link: 'https://www.flowmapp.com/',
    Description:
      'Plan sitemaps, user flows, wireframes, and project briefs collaboratively.',
    Tags: 'sitemaps, user flows, wireframing, collaboration tool, UX planning',
    Thumbnail: '/assets/thumbnails/flowmapp.com.webp',
    Category: 'Tool',
    Favicon: '/assets/favicons/flowmapp.com.png',
  },
  {
    Title: 'Imgly BG Removal',
    Link: 'https://img.ly/showcases/cesdk/background-removal/web/standalone',
    Description: 'Instant AI background removal for any image online.',
    Tags: 'background removal, ai background removal, remove image background, online background remover, photo background eraser',
    Thumbnail: '/assets/thumbnails/img.ly.webp',
    Category: 'Tool',
    Favicon: '/assets/favicons/img.ly.png',
  },
  {
    Title: 'Ad Gallery',
    Link: 'https://adgallery.xyz/',
    Description:
      'Curated gallery of display ads from major platforms and industries.',
    Tags: 'display ads, ad inspiration, digital marketing, ad gallery, creative advertising, ad design',
    Thumbnail: '/assets/thumbnails/adgallery.xyz.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/adgallery.xyz.png',
  },
  {
    Title: 'Brand Bird',
    Link: 'https://www.brandbird.app/',
    Description:
      'Transform screenshots into stunning graphics with mockups and 3D effects.',
    Tags: 'screenshot editor, mockup designs, social media graphics, 3D effects, templates, visual branding',
    Thumbnail: '/assets/thumbnails/brandbird.app.webp',
    Category: 'Utility',
    Favicon: '/assets/favicons/brandbird.app.png',
  },
  {
    Title: 'Build for Mars',
    Link: 'https://builtformars.com/ux-bites',
    Description:
      'Quick UX ideas, trends, and innovations to improve digital experiences.',
    Tags: 'UX inspiration, UX trends, product design, user experience ideas, UI patterns',
    Thumbnail: '/assets/thumbnails/builtformars.com.webp',
    Category: 'Learning',
    Favicon: '/assets/favicons/builtformars.com.png',
  },
  {
    Title: 'Chamjo',
    Link: 'https://chamjo.design/browse',
    Description: 'UX inspiration with local app screenshots from your region.',
    Tags: 'local app UX, UI inspiration, app screenshots, regional apps, UX research, design benchmarks',
    Thumbnail: '/assets/thumbnails/chamjo.design.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/chamjo.design.png',
  },
  {
    Title: 'CTA Examples',
    Link: 'https://ctaexamples.com/',
    Description:
      '100+ conversion-focused call to action examples for varied marketing goals.',
    Tags: 'ctas, call to action examples, button copy, marketing conversion, copywriting tips',
    Thumbnail: '/assets/thumbnails/ctaexamples.com.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/ctaexamples.com.png',
  },
  {
    Title: '3D Icons',
    Link: 'https://3dicons.co/',
    Description:
      'Free open-source 3D icon library ready for your design projects.',
    Tags: '3d icons, free design assets, vector graphics, icon library, creative resources',
    Thumbnail: '/assets/thumbnails/3dicons.co.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/3dicons.co.png',
  },
  {
    Title: 'Icon Buddy',
    Link: 'https://iconbuddy.com/',
    Description: 'Search and edit 200,000+ free open-source SVG icons.',
    Tags: 'icons, free design assets, svg library, ui elements, vector graphics',
    Thumbnail: '/assets/thumbnails/iconbuddy.com.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/iconbuddy.com.png',
  },
  {
    Title: 'View Port',
    Link: 'https://viewport-ui.design/',
    Description: 'Curated gallery of UI design patterns and experiences.',
    Tags: 'UI inspiration, design gallery, web/mobile UX, interaction patterns, visual showcase',
    Thumbnail: '/assets/thumbnails/viewport-ui.design.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/viewport-ui.design.png',
  },
  {
    Title: 'Shapes Gallery',
    Link: 'https://www.shapes.gallery/',
    Description: 'Free library of customizable SVG shapes for your projects.',
    Tags: 'svg shapes, design assets, vector graphics, creative resources, free illustrations',
    Thumbnail: '/assets/thumbnails/shapes.gallery.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/shapes.gallery.png',
  },
  {
    Title: 'Call To Inspiration',
    Link: 'https://calltoinspiration.com/',
    Description: 'Curated UI/UX design bits for serious creators.',
    Tags: 'design inspiration, UI components, UX patterns, design library, detail-driven',
    Thumbnail: '/assets/thumbnails/calltoinspiration.com.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/calltoinspiration.com.png',
  },
  {
    Title: 'Freelance Things',
    Link: 'https://www.freelancethings.co/',
    Description: 'Curated resources to level up your freelance journey.',
    Tags: 'freelance resources, tools for freelancers, community hub, articles & podcasts, freelance workflow',
    Thumbnail: '/assets/thumbnails/freelancethings.co.webp',
    Category: 'Resources',
    Favicon: '/assets/favicons/freelancethings.co.png',
  },
  {
    Title: 'My Codeless Website',
    Link: 'https://www.mycodelesswebsite.com/',
    Description: 'Discover how websites are built and reuse templates.',
    Tags: 'website design inspiration, site builders, examples gallery, codeless web design, no-code tools',
    Thumbnail: '/assets/thumbnails/mycodelesswebsite.com.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/mycodelesswebsite.com.png',
  },
  {
    Title: 'Sketch',
    Link: 'https://sketch.metademolab.com/',
    Description: 'Turn simple drawings into animations in seconds.',
    Tags: 'animation tool, AI animation, sketch to motion, creative tool, interactive design',
    Thumbnail: '/assets/thumbnails/sketch.metademolab.com.webp',
    Category: 'Tool',
    Favicon: '/assets/favicons/sketch.metademolab.com.png',
  },
  {
    Title: 'ProblemHunt',
    Link: 'https://problemhunt.pro/',
    Description:
      'Platform listing real-world problems people will pay to solve.',
    Tags: 'startup ideas, market problems, founders, co-founder matching, validation tool',
    Thumbnail: '/assets/thumbnails/problemhunt.pro.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/problemhunt.pro.png',
  },
  {
    Title: 'McStumble',
    Link: 'https://www.mcstumble.com/',
    Description: 'Discover fun websites and free browser-based tools.',
    Tags: 'website discovery, free tools, productivity, browser utilities, no-login tools',
    Thumbnail: '/assets/thumbnails/mcstumble.com.webp',
    Category: 'Inspiration',
    Favicon: '/assets/favicons/mcstumble.com.png',
  },
  {
    Title: 'Whisk',
    Link: 'https://www.labs.google/fx/tools/whisk',
    Description: 'Merge images, styles, generate scenes',
    Tags: 'image merge, styling, visuals, ai art',
    Thumbnail: '/assets/thumbnails/whisk.google.com.webp',
    Category: 'AI',
    Favicon: '/assets/favicons/whisk.google.com.png',
  },
  {
    Title: 'RoastMyPage',
    Link: 'https://www.roastmypage.com/',
    Description: 'AI roasts landing pages with fixes',
    Tags: 'landing page, feedback, optimization, conversion',
    Thumbnail: '/assets/thumbnails/roastmypage.com.webp',
    Category: 'AI',
    Favicon: '/assets/favicons/roastmypage.com.png',
  },
  {
    Title: 'MagickSEO',
    Link: 'https://magickseo.com/',
    Description: 'One-click SEO insights and improvements',
    Tags: 'seo, optimization, ranking, analytics',
    Thumbnail: '/assets/thumbnails/magickseo.com.webp',
    Category: 'AI',
    Favicon: '/assets/favicons/magickseo.com.png',
  },
  {
    Title: 'Potion',
    Link: 'https://potion.bestregards.me/',
    Description: 'AI Notion-style email builder',
    Tags: 'email builder, writing, automation, productivity',
    Thumbnail: '/assets/thumbnails/potion.bestregards.me.webp',
    Category: 'AI',
    Favicon: '/assets/favicons/potion.bestregards.me.png',
  },
  {
    Title: 'Youtube Summary',
    Link: 'https://www.youtubesummary.com/',
    Description: 'Instantly summarize any YouTube video',
    Tags: 'video summary, youtube, ai notes, learning',
    Thumbnail: '/assets/thumbnails/youtubesummary.com.webp',
    Category: 'AI',
    Favicon: '/assets/favicons/youtubesummary.com.png',
  },
  {
    Title: 'ImagePrompt',
    Link: 'https://imageprompt.org/',
    Description: 'Turn images into AI prompts',
    Tags: 'prompting, image to prompt, ai art, generation',
    Thumbnail: '/assets/thumbnails/imageprompt.org.webp',
    Category: 'AI',
    Favicon: '/assets/favicons/imageprompt.org.png',
  },
  {
    Title: 'Rabbithole',
    Link: 'https://www.rabbithole.chat/',
    Description: 'Infinite canvas AI chat workspace',
    Tags: 'canvas chat, brainstorming, ai workspace, thinking',
    Thumbnail: '/assets/thumbnails/rabbithole.chat.webp',
    Category: 'AI',
    Favicon: '/assets/favicons/rabbithole.chat.png',
  },
  {
    Title: 'Mind Map Wizard',
    Link: 'https://mindmapwizard.com/',
    Description: 'Generate mind maps in seconds',
    Tags: 'mind mapping, planning, ideas, structure',
    Thumbnail: '/assets/thumbnails/mindmapwizard.com.webp',
    Category: 'AI',
    Favicon: '/assets/favicons/mindmapwizard.com.png',
  },
  {
    Title: 'WTFood',
    Link: 'https://whatthefood.io/',
    Description: 'Detect food and track calories',
    Tags: 'food detect, calories, nutrition, health',
    Thumbnail: '/assets/thumbnails/whatthefood.io.webp',
    Category: 'AI',
    Favicon: '/assets/favicons/whatthefood.io.png',
  },
]

export default bookmarks
